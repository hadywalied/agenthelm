# 📋 Implementation Plan — AgentHelm v0.3.0 (DSPy + Multi-Agent)
...
├── core/
│   ├── __init__.py
│   ├── tool.py              # @tool decorator + TOOL_REGISTRY (from AgentHelm)
│   ├── event.py             # ExecutionEvent model (enhanced)
│   ├── tracer.py            # ExecutionTracer (from AgentHelm)
│   ├── handlers.py          # ApprovalHandler, CliHandler (from AgentHelm)
│   ├── cost.py              # NEW: CostTracker, TokenUsage
│   └── storage/
│       ├── __init__.py
│       ├── base.py          # BaseStorage ABC (from AgentHelm)
│       ├── json_storage.py  # JSON file storage (from AgentHelm)
│       └── sqlite_storage.py # SQLite storage (from AgentHelm)
├── agent/
│   ├── __init__.py
│   ├── base.py              # Agent base class (NEW: DSPy-native)
│   └── registry.py          # Agent registry
├── memory/
│   ├── __init__.py
│   ├── hub.py               # MemoryHub
│   ├── context.py           # MemoryContext (per-session)
...
```python
# nexus/core/tool.py — Ported from AgentHelm
...
```python
# nexus/core/event.py — Ported from AgentHelm
...
```python
# nexus/core/cost.py — NEW component
...
- ✅ `@tool` decorator ported from AgentHelm
- ✅ `ExecutionTracer` ported with rollback support
- ✅ `ApprovalHandler` hierarchy ported
- ✅ Storage backends ported (JSON, SQLite)
- ✅ NEW: `CostTracker` with model pricing
- ✅ NEW: Enhanced `ExecutionEvent` with cost tracking
- ✅ Unit tests for all ported components
...
### 2.2 Semantic Memory (Qdrant-backed)
```python
...
- Working `SemanticMemory` with Qdrant
...
### 3.4 PlannerAgent & Plan Schema (NEW)

```python
# agenthelm/planning/schema.py
...
class Plan(BaseModel):
    ...
    @classmethod
    def from_yaml(cls, path: str) -> "Plan":
        with open(path) as f:
...
```python
# agenthelm/planning/planner.py
class PlannerAgent(Agent):
    """Generates execution plans from tasks (single-round)."""
    
    class Signature(dspy.Signature):
        """Generate a structured execution plan."""
        task: str = dspy.InputField(desc="The user's task")
        available_agents: str = dspy.InputField(desc="JSON list of agent capabilities")
        plan: str = dspy.OutputField(desc="YAML execution plan")
    
    def __init__(self, agents: list[Agent]):
        super().__init__(name="planner", description="Generates execution plans")
        self.agents = agents
        self.generate = dspy.ChainOfThought(self.Signature)
    
    def forward(self, context: MemoryContext, task: str) -> Plan:
        # Gather agent capabilities
        capabilities = [
            {"name": a.name, "description": a.description, **a.get_signature()}
            for a in self.agents
        ]
        
        # Generate plan via DSPy
        result = self.generate(
            task=task,
            available_agents=json.dumps(capabilities)
        )
        
        # Parse and return
        return Plan.model_validate(yaml.safe_load(result.plan))
```
...
- **NEW**: `Plan` Pydantic model (YAML serializable)
- **NEW**: `PlannerAgent` for single-round plan generation
...
**This is a key AgentHelm feature** — automatic rollback when a step fails:
...
```python
# nexus/core/tracer.py — Enhanced from AgentHelm
...
> **Decision**: Option B — CLI + External Tracing UI (Jaeger/Tempo)
> No custom web dashboard in v0.3.0. Use existing tracing visualization tools.
...
@cli.command()
@click.argument("task")
@click.option("--agents", "-a", multiple=True, required=True, help="Agents to use")
@click.option("--trace", is_flag=True, help="Enable tracing")
def run(task: str, agents: tuple[str], trace: bool):
...
@cli.command()
@click.option("--port", default=16686, help="Jaeger UI port")
def traces(port: int):
    """Open the Jaeger tracing UI in your browser."""
    import webbrowser
    url = f"http://localhost:{port}"
    webbrowser.open(url)
    console.print(f"[green]✓ Opened Jaeger UI at {url}[/]")
    console.print("[dim]Make sure Jaeger is running: docker run -d -p 16686:16686 jaegertracing/all-in-one[/]")

@cli.command()
@click.argument("task")
@click.option("--from-file", "-f", type=click.Path(exists=True), help="Load plan from YAML file")
@click.option("--save", "-s", type=click.Path(), help="Save generated plan to file")
@click.option("--auto-approve", is_flag=True, help="Skip approval and execute immediately")
def plan(task: str, from_file: str, save: str, auto_approve: bool):
    """Generate and execute a plan for a task."""
    from agenthelm.planning import PlannerAgent, Plan, PlanExecutor
    
    # Load or generate plan
    if from_file:
        plan = Plan.from_yaml(from_file)
        console.print(f"[blue]Loaded plan from {from_file}[/]")
    else:
        planner = PlannerAgent(agents=AgentRegistry.list_all())
        plan = planner.generate(task=task)
    
    # Display plan
    console.print(Panel(plan.to_yaml(), title="📋 Generated Plan", border_style="blue"))
    
    # Save if requested
    if save:
        with open(save, "w") as f:
            f.write(plan.to_yaml())
        console.print(f"[green]✓ Saved plan to {save}[/]")
    
    # Approval flow
    if not auto_approve:
        choice = console.input("[a]pprove / [e]dit / [r]egenerate / [q]uit: ")
        if choice == "q":
            return
        elif choice == "e":
            # Open in editor
            click.edit(plan.to_yaml())
        elif choice == "r":
            plan = planner.generate(task=task)
    
    # Execute
    executor = PlanExecutor(memory_hub=hub, tracer=tracer)
    result = asyncio.run(executor.execute(plan))
    console.print("[green]🎉 Plan executed successfully![/]")
```

### 5.2 OpenTelemetry Integration
```python
# agenthelm/tracing.py
...
class AgentHelmTracer:
    ...
    def __init__(self, service_name: str = "agenthelm", exporter: str = "jaeger"):
        self.provider = TracerProvider()
        trace.set_tracer_provider(self.provider)
        
        # Add exporter based on config
        if exporter == "jaeger":
            exp = JaegerExporter(agent_host_name="localhost", agent_port=6831)
        elif exporter == "otlp":
        ...
        self.provider.add_span_processor(BatchSpanProcessor(exp))
        self.tracer = trace.get_tracer(service_name, "0.3.0")
    
    @contextmanager
    def trace_agent(self, agent_name: str, task: str):
...
graph LR
    ...
    subgraph OpenTelemetry
        Tracer[AgentHelmTracer]
        Exporter[Jaeger Exporter]
    end
    
    subgraph External UI
        Jaeger[Jaeger UI<br/>localhost:16686]
    end
...
**Deliverables:**
- Full CLI with `run`, `list-agents`, `describe`, `traces`, **`plan`** commands
- `agenthelm plan "task"` generates plan with approval flow
- `agenthelm plan --from-file plan.yaml` loads user-defined plans
- `agenthelm traces` opens Jaeger UI in browser
- OpenTelemetry tracing integrated into agents, tools, and memory
- Trace export to Jaeger (default) or OTLP (configurable)
...
```
tests/
├── conftest.py          # Fixtures (mock LLM, memory backends)
├── unit/
│   ├── test_agent.py
│   ├── test_memory.py
...
**Testing Strategy:**
- Unit tests: Mock DSPy LLM calls, mock Redis/Qdrant
...
```
examples/
├── simple_qa/           # Single agent Q&A
├── research_pipeline/   # Multi-agent research workflow
└── parallel_analysis/   # Parallel agent execution
```
...
| **Qdrant** | Semantic memory | `docker run -d -p 6333:6333 qdrant/qdrant` |
...
| `CostTracker`, `TokenUsage` | — | **NEW** |
| `Agent` (DSPy base) | — | **NEW** |
| `MemoryHub`, `MemoryContext` | — | **NEW** |
| `ShortTermMemory`, `SemanticMemory` | — | **NEW** |
| `Pipeline`, `Parallel`, `Orchestrator` | — | **NEW** |
| CLI (`agenthelm`) | — | **NEW** |
