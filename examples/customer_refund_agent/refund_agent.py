        ...
        if not os.path.exists(db_file):
            sample_orders = {
                "ORD-1001": {
                    ...
                    "status": "delivered",
                    "items": [
                        {"id": "ITEM-1", "name": "Premium Headphones", "price": 75.50, "quantity": 1}
                    ],
                ...
                "ORD-1002": {
                    ...
                    "status": "delivered",
                    "items": [
                        {"id": "ITEM-2", "name": "Wireless Charger", "price": 45.99, "quantity": 1},
                        {"id": "ITEM-3", "name": "Phone Case", "price": 25.00, "quantity": 2},
                        {"id": "ITEM-4", "name": "Screen Protector", "price": 15.00, "quantity": 2}
                    ],
                ...
                "ORD-1003": {
                    ...
                    "status": "processing",
                    "items": [
                        {"id": "ITEM-5", "name": "Smart Watch", "price": 250.00, "quantity": 1}
                    ],
                    "payment_method": "credit_card"
                }
            }
        ...
        if not os.path.exists(db_file):
            sample_customers = {
                ...
                "CUST-002": {
                    ...
                    "account_status": "active",
                    "refund_history": [
                        {"order_id": "ORD-999", "amount": 30.00, "date": "2023-04-10", "reason": "damaged item"}
                    ]
                },
                "CUST-003": {
                    "name": "Bob Johnson",
...
class PaymentProcessor:
    def process_refund(self, order_id: str, amount: float, payment_method: str) -> Dict[str, Any]:
...
class EmailApprovalHandler(ApprovalHandler):
    ...
    def request_approval(self, tool_name: str, arguments: dict) -> bool:
        ...
        if tool_name == "process_refund" and arguments.get("refund_amount", 0) > 500:
            logger.info("Approval denied: Amount exceeds maximum allowed for auto-approval")
            return False
...
def process_refund(order_id: str, customer_id: str, refund_amount: float, reason: str) -> Dict[str, Any]:
    """
...
def send_refund_confirmation(customer_email: str, order_id: str, refund_amount: float, refund_id: str) -> Dict[str, Any]:
    """
...
def log_audit_record(action: str, details: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to write audit record: {str(e)}"
        }
